...
namespace Domari
{
    public class StudentskiDom
    {
        ...
        {
            var sobaEdit = Sobe.Find(s => s.BrojSobe == soba.BrojSobe);
            if (sobaEdit.Kapacitet == noviKapacitet)
            {                                                                       //Funkciju uradio Muamer Bandic
                sobaEdit.Stanari.Clear();
            }
            else if (sobaEdit.Kapacitet < noviKapacitet)
            { sobaEdit.Stanari.RemoveRange(noviKapacitet - 1, noviKapacitet - sobaEdit.Stanari.Count); }

            sobaEdit.Kapacitet = noviKapacitet;
        }
